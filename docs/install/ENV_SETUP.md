...
# NETBOX_API_URL=
# NETBOX_API_TOKEN=
# Path to a CA bundle for verifying the NetBox TLS cert. If UNSET, certificate
# verification is DISABLED (the API token then travels over an unverified
# connection). Set this to your internal CA to enable verification.
# NETBOX_CA_BUNDLE=

# --- API security knobs ---
# Allowed CORS origins (comma-separated). Defaults to '*', which combined with
# credentials is permissive; set to the actual frontend origin in production.
# CORS_ALLOW_ORIGINS=https://example.host
# JWT lifetime in seconds. Defaults to 1 year (deliberate for always-on signage
# displays); lower it (e.g. 3600) for tighter security and rely on token refresh.
# JWT_LIFETIME_SECONDS=

# BrightSign player credentials. If unset, falls back to the vendor defaults
# admin/avm — set explicitly so an unset env never uses guessable creds.
# BRIGHTSIGN_USERNAME=
# BRIGHTSIGN_PASSWORD=
